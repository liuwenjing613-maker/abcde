...
Usage:
    PYTHONPATH=src python scripts/exp_deep_residual.py \
        ...
        --output-dir artifacts/exp/deep_residual \
        --device cuda
"""
...
from ccac.experiments.deep_residual import DeepResidualConfig, train_deep_residual


def main():
    p = argparse.ArgumentParser(description="Trial 2: Deep residual architecture")
    p.add_argument("--dataset-path", type=str, default="datasets")
    p.add_argument("--output-dir", type=str, default="artifacts/exp/deep_residual")
    ...
    result = train_deep_residual(bc, dc)
