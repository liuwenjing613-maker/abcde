...
from ccac.metrics import extended_classification_report, robust_score as _robust_score
...
class BaselineConfig:
    ...
    patience: int = 12
    num_folds: int = 5
    num_workers: int = 0  # 0 = single-process DataLoader; set >0 to enable workers
    seed: int = 42
    device: str = "cuda"
...
def train_anxiety_baseline(config: BaselineConfig) -> dict[str, Any]:
    ...
    for fold_id, (train_idx, val_idx) in enumerate(fold_indices, start=1):
        ...
        train_loader = DataLoader(train_dataset, batch_size=config.batch_size, shuffle=True, num_workers=workers)
        val_loader = DataLoader(val_dataset, batch_size=config.batch_size, shuffle=False, num_workers=workers)
        ...
        class_weights = _class_weights(labels[train_idx], len(label_mapping), config.class_weight_power)
...
def _write_release_test_predictions(
    ...
    for state in fold_states:
        ...
        loader = DataLoader(dataset, batch_size=config.batch_size, shuffle=False, num_workers=_resolve_num_workers(config.num_workers))
        model = LongitudinalAnxietyModel(
            input_dim=input_dim,
...
def _build_release_features(
    ...
    # Pre-scan which pooled files exist to avoid per-file stat() calls
    audio_root = dataset_root / split / audio_feature_name
    video_root = dataset_root / split / video_feature_name
    audio_pooled = _scan_pooled_cache(audio_root)
    video_pooled = _scan_pooled_cache(video_root)

    features = np.zeros((len(frame), len(STAGES), len(CLIP_TYPES), input_dim), dtype=np.float32)
    clip_mask = np.zeros((len(frame), len(STAGES), len(CLIP_TYPES)), dtype=bool)

    # Build task list: one per subject × stage × clip
    tasks: list[tuple[int, int, int, tuple[str, str, str]]] = []
    ...
    # Parallel file I/O via threads (GIL released during disk reads)
    max_workers = min(32, (os.cpu_count() or 4) * 2)
    with ThreadPoolExecutor(max_workers=max_workers) as pool:

        def _load_task(task):
            row_index, stage_index, clip_index, subject_parts, stage, clip_type = task
            audio = _load_release_vector_cached(
                audio_root, audio_pooled, subject_parts, stage, clip_type, audio_dim
            )
            video = _load_release_vector_cached(
                video_root, video_pooled, subject_parts, stage, clip_type, video_dim
            )
            return row_index, stage_index, clip_index, audio, video

        for row_index, stage_index, clip_index, audio, video in pool.map(_load_task, tasks):
            ...
            clip_mask[row_index, stage_index, clip_index] = bool(
                np.isfinite(audio).any() or np.isfinite(video).any()
            )

    return np.nan_to_num(features, nan=0.0, posinf=0.0, neginf=0.0), clip_mask, input_dim


def _scan_pooled_cache(feature_root: Path) -> dict[tuple[str, str, str, str, str], Path]:
    """Pre-scan all pooled files so per-clip lookups are O(1) dict hits instead of
    repeated Path().exists() calls."""
    cache: dict[tuple[str, str, str, str, str], Path] = {}
    for path in feature_root.rglob("pooled.npy"):
        # path: <root>/school/class/person/stage/clip/pooled.npy
        parts = path.relative_to(feature_root).parts
        if len(parts) >= 5:
            cache[(parts[0], parts[1], parts[2], parts[3], parts[4])] = path
    for path in feature_root.rglob("pooled.json"):
        parts = path.relative_to(feature_root).parts
        if len(parts) >= 5:
...
def _load_release_vector_cached(
    feature_root: Path,
    pooled_cache: dict[tuple[str, str, str, str, str], Path],
    subject_parts: tuple[str, str, str],
    stage: str,
    clip_type: str,
    target_dim: int,
) -> np.ndarray:
    key = (subject_parts[0], subject_parts[1], subject_parts[2], stage, clip_type)
    path = pooled_cache.get(key)
    if path is None:
        return np.zeros(target_dim, dtype=np.float32)
    values = _read_feature_vector(path)
...
def _evaluate(
    ...
    metrics = _extended_metrics(labels, predictions, probabilities, num_classes)
...
def _classification_metrics(labels: np.ndarray, predictions: np.ndarray) -> dict[str, float]:
    """Legacy metric function — kept for backward compatibility.

    Prefer _extended_metrics() which includes macro_auc, QWK, and coverage checks.
    """
    return {
...
def _extended_metrics(
    ...
    """Full metrics including new framework: macro_auc, QWK, robust_score.

    Combines legacy metrics with the three-dimensional evaluation from ccac.metrics.
    """
    # Legacy metrics
    legacy = _classification_metrics(labels, predictions)

    # New metrics (requires probabilities)
    if len(probabilities) > 0 and probabilities.shape[1] == num_classes:
        from ccac.metrics import macro_auc, min_class_recall, quadratic_weighted_kappa, robust_score

        auc = macro_auc(probabilities, labels, num_classes)
        qwk = quadratic_weighted_kappa(labels, predictions, num_classes)
        min_rec = min_class_recall(labels, predictions, num_classes)
        rs = robust_score(probabilities, labels, num_classes)
    else:
        auc = 0.5
        qwk = 0.0
        min_rec = 0.0
        rs = {"robust_score": 0.25}
    ...
    return {
        **legacy,
        ...
        "robust_score": rs["robust_score"] if isinstance(rs, dict) else rs,
    }
